...
Use p(yes) = 0.5 which will give us the median willingness to pay, mode age is tothirty, mode income is one percent, and the average NEP (38.366).
...
bid_zero <- (intercept + age_30 + inc_onepercent + (nep*nep_avg) + (risk*0) - med_vote)/bid
#241

#Bid if risk reduction is four:
bid_four <- (intercept + age_30 + inc_onepercent + (nep*nep_avg) + (risk*4) - med_vote)/bid
#244
```

Value of one whale = 244 - 4241 = $3 per Santa Barbara resident
...
###4
Assume median prob of voting yes is ($p = 0.5$). Use mode income bracket (one_percent), mode age bracket (to 30), and average NEP (38.366) to find WTP:
...
Average WTP in Santa Barbara households is $283.

###5

Total Benefits:
$283*150,000 = 42,450,000 $

Cost = $7 million

Net Benefits:
$42,450,000 - $7,000,000 = $35,450,000
