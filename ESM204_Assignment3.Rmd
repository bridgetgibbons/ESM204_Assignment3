...
Assume p(yes) is ($p = 0.714$), mode age is tothirty, mode income is one percent, and the average NEP (38.366).

WTP:
$0.714 = `r round(intercept, digits = 4)` + `r round(age_30, digits=4)`(Age~to~30) + `r round(inc_onepercent, digits=4)`(Income~One~Percnet) + `r round(nep, digits=4)`(NEP) - `r round(bid, digits=4)`(Bid) + `r round(risk, digits=4)`(0)$
...
bid_zero <- (intercept + age_30 + inc_onepercent + (nep*nep_avg) + (risk*0) - avg_vote)/bid
#40.8

#Bid if risk reduction is four:
bid_four <- (intercept + age_30 + inc_onepercent + (nep*nep_avg) + (risk*4) - avg_vote)/bid
#43.5
```

Value of one whale = 43.5 - 40.8 = $2.7 per Santa Barbara resident
...
###4
Assume average prob of voting yes is ($p = 0.7$). Use mode income bracket (one_percent), mode age bracket (to 30), and average NEP (38.366) to find WTP:

$`r avg_vote` = `r round(intercept, digits = 4)` + `r round(age_30, digits=4)`(Age~to~30) + `r round(age_30, digits=4)`(Age~to~30) + `r round(nep, digits=4)`(NEP) - `r round(bid, digits=4)`(Bid) + `r round(risk, digits=4)`(60)$  

```{r, include = FALSE}

avg_bid_sb <- (intercept + age_30 + inc_onepercent + (nep*nep_avg) + (risk*60) - avg_vote)/bid

#82.5

```

Average WTP in Santa Barbara households is $82.5.

###5

Total Benefits:
$82.5*150,000 = 12,375,000 $

Cost = $7 million

Net Benefits:
$12,375,000 - $7,000,000 = $5,375,000
...
b) $12,375,000? or 15 whales times 2.7 times 150,000 people?
